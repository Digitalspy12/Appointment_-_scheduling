...
import { format, formatISO, isBefore, isSameDay, startOfDay } from 'date-fns'
...
export async function getAvailableSlots(dateStr: string): Promise<TimeSlot[]> {
    ...
    // Past dates or weekends shouldn't be queried normally, but UI protects against it
    ...
    while (currentSlotStart + settings.slot_duration_mins <= endMins) {
        ...
        // Check approved appointments (we treat them like busy blocks)
...
export async function createBooking(request: BookingRequest): Promise<BookingResult> {
    ...
    const { error } = await supabase.from('appointment').insert({
        ...
        appointment_date: format(new Date(request.appointmentDate), 'yyyy-MM-dd'),
